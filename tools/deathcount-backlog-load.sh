#!/usr/bin/env bash
# deathcount-backlog-load.sh — hook SessionStart : charge les tickets #deathcount OUVERTS depuis le
# backlog serveur (source de vérité `backlog.d/deathcount.md`, rendu live sur le hub Asimov).
#
...
BL="/root/.openclaw/server-context/backlog.d/deathcount.md"

echo "[deathcount] Tickets OUVERTS (source de vérité : $BL — rendu sur asimov.bourdat.fr) :"
...
if [ -n "$out" ]; then
  ...
  echo "  dans backlog.d/deathcount.md ; AJOUT via 'oc-backlog add' (jamais l'ancien monolithe backlog.md)."
else
